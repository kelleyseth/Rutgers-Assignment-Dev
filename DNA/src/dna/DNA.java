...
public class DNA {
     ...
     * 
     * @param profile The profile of that the method will compute the STRs array for
    ...
    public ArrayList<Profile> findPossibleParents ( STR[] S1_STRs, STR[] S2_STRs ) {
        ...
        /* FIX THIS METHOD */
         ...
         for ( int i = 0; i < database.length; i++ ) {
            ...
            if (identicalSTRs(database[i].getCSTRs(), S_STRs)) {
         ...
         for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
            ...
            for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
                ...
                if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                    ...
                    if ( punnetSquare(possibleParent2.get(p2).getCSTRs(), S_STRs, possibleParent1.get(p1).getCSTRs(), S_STRs)) { // bottom right
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent2.get(p2).getCSTRs(), S_STRs, possibleParent1.get(p1).getCSTRs(), S_STRs)) {//top left
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent2.get(p2).getCSTRs(), S_STRs, possibleParent1.get(p1).getCSTRs(), S_STRs)) { //bottom left
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent2.get(p2).getCSTRs(), S_STRs, possibleParent1.get(p1).getCSTRs(), S_STRs)) { //top right
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    }
                }
