 ...
 * @author Kevin Wayne
 */
public final class Picture implements ActionListener, MouseMotionListener {
    private BufferedImage image; // the rasterized image
    private JDialog frame; // on-screen view
    private String filename; // name of file
    private boolean isOriginUpperLeft = true; // location of origin
    private final int width, height; // width and height
    ArrayList<Integer> coordsX = new ArrayList<>(); // array of x coords when drawing
    ArrayList<Integer> coordsY = new ArrayList<>(); // array of y coords when drawing
    ...
    /**
     * Returns x coord of what is drawn on the picture
     * 
     * @return Integer Arraylist of x coords
     */
    ...
    /**
     * Returns y coords of what is drawn on the picture
     * 
     * @return Integer Arraylist of y coords
     */
    ...
    /**
     * Draws on the picture and records what is being drawn on
     * Adds all x and y values to X and Y Arraylists
     */
    @Override
    public void mouseDragged(MouseEvent e) {
        ...
        int r = 25; // Radius of the circle/point.
        int x = cx - (r / 2); // Position X (mouse will be in the center of the point)
        int y = cy - (r / 2); // Position Y (mouse will be in the center of the point)

        // Loops to get all points inside the circle
        for (int xi = cx - r; xi < cx + r; xi++) {
            for (int yi = cy - r; yi < cy + r; yi++) {
                int dx = xi - cx;
                int dy = yi - cy;
                // Ensure indicies are within the picture dimensions
                if ((inRange(xi, width()) && (inRange(yi, height())))) {
        ...
        Graphics g = frame.getContentPane().getGraphics(); // Getting the Graphic object
        Color b = new Color(255, 0, 0, 10);
        g.setColor(b);
    ...
    public boolean inRange(int i, int bound) {
        if ((i >= 0) && (i < bound))
            return true;
        return false;
    }
